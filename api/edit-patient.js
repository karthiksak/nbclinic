export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Parse lines and update the matched row
    let lines = currentContent.split(/\r?\n/);
    let updated = false;
    
    for (let i = 1; i < lines.length; i++) {
        // Simple check for the ID at the start of the row
        if (lines[i].startsWith(`${id},`) || lines[i].startsWith(`"${id}",`)) {
            lines[i] = newRow;
            updated = true;
            break; // Assuming ID is unique, we replace the first match only
        }
    }
    
    if (!updated) {
       return res.status(404).json({ error: 'Patient ID not found in database. Cannot edit.' });
    }
    ...
    const putRes = await fetch(url, {
        ...
        body: JSON.stringify({
            message: `Admin: EDITED patient ${name} via Tracker Module`,
